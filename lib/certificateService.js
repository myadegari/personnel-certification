...
export async function processCertificateGeneration({
  ...
  if (nextNumber > 999) {
    await Indicator.updateOne({ pattern }, { $inc: { lastNumber: -1 } });
    throw new Error(
      `Certificate number capacity for pattern "${pattern}" has been reached.`
    );
  }

  const certNumber = `${pattern}/${nextNumber}`;
  ...
  try {
    ...
    return {
      ...
      certificateUrl: `${NEXTJS_APP_URL}/verify/${enrollmentId}`,
    };
  } catch (error) {
